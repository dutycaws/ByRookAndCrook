...
| Community NPC expression sprites | `.local/media/source-masters/community-npcs/expression-sprites/<npc-id>/<slot>.png` | Ignored; never seed or commit source bytes | Creator Studio validates and normalizes an author-chosen PNG to a private master plus private alpha-WebP derivative. Optional runtime slots resolve to the selected Neutral sprite when no expression is selected. |
...
Community NPC expression sprites use a separate private path. `npm run fixtures:users:local` always provisions the local `community-npc-portrait-masters` PNG bucket and `community-npc-portraits` WebP bucket, including on a fresh checkout with no local art. It deliberately does not scan, upload, or register `.local/media/source-masters/community-npcs/expression-sprites/`: a source becomes a candidate only through the authenticated Creator Studio upload action, which validates the decoded image, creates a 1024 × 1536 transparent master and a max-500,000-byte WebP derivative, verifies both objects, and records ownership/provenance/Neutral anchor atomically. If persistence fails, it removes the just-written objects. This preserves the distinction between a developer’s private file and an author’s reviewed game asset.

For local manual testing, copy a PNG to the ignored expression-sprites directory, run `npm run fixtures:users:local` to provision Storage and pilot identities, then upload it in Creator Studio into the intended slot. Start with Neutral. Missing optional slots remain a resolved Neutral fallback. Changing Neutral clears optional selections; their candidates remain reviewable as stale and require an explicit confirmation before being selected again. Discard deletes only an unselected, unpinned editable-draft candidate. Moderation quarantine or purge follows the same deletion queue for masters and derivatives while preserving audit-safe metadata.
