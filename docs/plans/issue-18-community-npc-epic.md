...
- `npm run check -- --output human`, `npm run build`, `npm run db:types:check`, `npm run secrets:audit`, `npm run npc:catalog:check`, and `git diff --check` pass. The working-tree inventory contains no new image or binary media.
