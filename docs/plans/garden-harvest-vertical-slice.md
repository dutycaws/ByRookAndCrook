...
The next product slice should consume a stored ingredient batch in a 30-second brewing minigame, enforce one crafting minigame per in-game day, and atomically persist the resulting beverage and social-card effects. After that, serving can connect prepared-item quality to NPC payment and story progression. These are distinct later deliverables, grounded in the PDD; raw ingredients must not be served as prepared drinks or foods.
