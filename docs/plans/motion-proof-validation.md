...
`BreweryMotionProof.svelte` and `BakeryMotionProof.svelte` are presentation components. They own transient pointer state, animation phase, and visual feedback. They do not own a brew session, bakery session, score, reward, item, or save revision.
