...
reset role;
create temporary table garden_occupancy_context on commit drop as
  select id as save_id
  from public.tavern_saves
  where user_id='16200000-0000-4000-8000-000000000001';

select is((select count(*) from public.garden_cells c
  join garden_occupancy_context ctx on ctx.save_id=c.save_id
...
select throws_like($$
  ...
  select c.save_id,c.id,'garden-apiary-v1','clover','seedling',1
  from public.garden_cells c join garden_occupancy_context ctx on ctx.save_id=c.save_id
...
select throws_like($$
  ...
  select c.save_id,c.id,1
  from public.garden_cells c join garden_occupancy_context ctx on ctx.save_id=c.save_id
  where c.layout_key='c2';
  set constraints all immediate
...
select throws_like($$
  ...
  where layout_key='c11' and save_id=(select save_id from garden_occupancy_context);
  set constraints all immediate
$$,'%occupancy does not match kind plant%',
...
select throws_like($$
  ...
  where layout_key='c0' and save_id=(select save_id from garden_occupancy_context);
  set constraints all immediate
...
select lives_ok($$
  ...
  join garden_occupancy_context ctx on ctx.save_id=source.save_id
  ...
  from public.garden_cells c0
  join public.garden_cells c2 on c2.save_id=c0.save_id
  join garden_occupancy_context ctx on ctx.save_id=c0.save_id
...
select is((select count(*) from public.apiary_hives h join public.garden_cells c
  on c.save_id=h.save_id and c.id=h.cell_id
  join garden_occupancy_context ctx on ctx.save_id=c.save_id
  where c.layout_key='c0' and c.kind='beehive'),
...
select is((select count(*) from public.garden_plants p join public.garden_cells c
  on c.save_id=p.save_id and c.id=p.cell_id
  join garden_occupancy_context ctx on ctx.save_id=c.save_id
  where c.layout_key='c2' and c.kind='plant'),
