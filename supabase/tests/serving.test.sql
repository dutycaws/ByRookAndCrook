...
select throws_ok($$delete from public.serving_events$$, '42501', null, 'players cannot erase consumption');
...
select is(array(select gold_earned from public.serving_events where patron_key='lira' order by committed_revision),
  array[2,5,8,12,18,28,45], 'all seven Lira prices match the prototype');
select is(array(select gold_earned from public.serving_events where patron_key='torvin' order by committed_revision),
  array[1,3,6,10,16,25,40], 'all seven Torvin prices match the prototype');
select is(array(select relationship_change from public.serving_events where patron_key='lira' order by committed_revision),
  array[-4,-2,3,3,6,6,6], 'quality determines base relationship effect');
select is(array(select arc_change from public.serving_events where patron_key='lira' order by committed_revision),
...
select is(jsonb_array_length(public.get_bar_snapshot()->'cards'), 1, 'unplayed card remains available');
...
create trigger test_reject_serving before insert on public.serving_events for each row execute function public.test_reject_serving();
set local role authenticated;
select throws_ok($$select public.serve_beverage((select id from public.tavern_saves),'lira',
...
select is(jsonb_array_length(public.get_bar_snapshot()->'cards'),1,'failure does not consume a card');
select is((select revision from public.tavern_saves),15::bigint,'failure does not advance revision');
reset role;
drop trigger test_reject_serving on public.serving_events;
...
select is((select gold_earned from public.serving_events where committed_revision=16),4,'card multiplies patron payment');
select is((select relationship_change from public.serving_events where committed_revision=16),6,'card adds its stored relationship bonus');
select is((select arc_change from public.serving_events where committed_revision=16),0,'cards do not change historical story progress');
...
select is(jsonb_array_length(public.get_bar_snapshot()->'cards'),0,'played card is consumed once');
select throws_ok($$select public.serve_beverage((select id from public.tavern_saves),'torvin',
  '63000000-0000-4000-8000-000000000016','64000000-0000-4000-8000-000000000001',
  '65000000-0000-4000-8000-000000000016',16)$$, 'PT409','This social card has already been played','a card cannot be played twice');
...
select is((select relationship_change from public.serving_events where committed_revision=17),1,'receipt records actual clamped relationship change');
select is((select arc_change from public.serving_events where committed_revision=17),0,'receipt records no extra story progress beyond completion');
...
select is((select count(*) from public.serving_events),0::bigint,'RLS hides serving history');
...
update public.beverages set quality_index=6 where save_id=(select id from public.tavern_saves where user_id=auth.uid());
...
update public.beverages set quality_index=0 where save_id=(select id from public.tavern_saves where user_id=auth.uid());
