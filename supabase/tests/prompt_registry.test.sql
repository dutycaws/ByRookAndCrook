...
select ok((
  ...
  join private.prompt_release_entries entry on entry.release_id=active.release_id and entry.prompt_key='procedural.proposer'
  join private.prompt_revisions revision on revision.id=entry.revision_id
) like '%Quest creation and quest updates belong exclusively to the canonical quest-transition workflow%', 'active procedural prompt defers all quest authority to canonical transitions');
