...
reset role;
create temporary table apiary_day_test_context as
select id as save_id
from public.tavern_saves
where user_id='16800000-0000-4000-8000-000000000001';
grant select on apiary_day_test_context to authenticated;
update public.garden_plants set lifecycle='growing',growth_progress=10,flowering_days_remaining=0
where save_id=(select save_id from apiary_day_test_context);
update public.garden_plants set lifecycle='flowering',growth_progress=75,flowering_days_remaining=3
where save_id=(select save_id from apiary_day_test_context)
  and cell_id=(select id from public.garden_cells where save_id=(select save_id from apiary_day_test_context) and layout_key='c1');
create temporary table single_colony_plan as
select private.resolve_garden_day((select save_id from apiary_day_test_context),1) plan;
...
select (select save_id from apiary_day_test_context),id,1 from public.garden_cells where save_id=(select save_id from apiary_day_test_context) and layout_key='c3';
select private.sync_garden_cells((select save_id from apiary_day_test_context),
  array[(select id from public.garden_cells where save_id=(select save_id from apiary_day_test_context) and layout_key='c3')]);
insert into public.apiary_colonies(save_id,hive_id,established_day)
select (select save_id from apiary_day_test_context),h.id,1 from public.apiary_hives h
join public.garden_cells c on c.id=h.cell_id and c.save_id=h.save_id where h.save_id=(select save_id from apiary_day_test_context) and c.layout_key='c3';
...
select is((select plan->>'planFingerprint' from two_colony_plan),
  private.resolve_garden_day((select save_id from apiary_day_test_context),1)->>'planFingerprint',
  'repeated apiary projection keeps the same plan fingerprint');

update public.garden_plants set lifecycle='growing',growth_progress=10,flowering_days_remaining=0
where save_id=(select save_id from apiary_day_test_context);
update public.garden_weather set weather_key='rainy'
where save_id=(select save_id from apiary_day_test_context) and day_number=1;
...
where ac.save_id=(select save_id from apiary_day_test_context) and ac.hive_id=h.id and h.save_id=ac.save_id and c.save_id=h.save_id and c.layout_key='c2';
update public.apiary_colonies ac set nosema_pressure=40
from public.apiary_hives h join public.garden_cells c on c.id=h.cell_id
where ac.save_id=(select save_id from apiary_day_test_context) and ac.hive_id=h.id and h.save_id=ac.save_id and c.save_id=h.save_id and c.layout_key='c3';
...
select is((select (colony->>'varroa')::integer from disease_plan,
  ...
  where (colony->>'hiveId')::uuid=(select h.id from public.apiary_hives h join public.garden_cells c on c.id=h.cell_id and c.save_id=h.save_id where h.save_id=(select save_id from apiary_day_test_context) and c.layout_key='c2')),
...
set constraints all deferred;
update public.garden_cells set unlocked=true where save_id=(select save_id from apiary_day_test_context) and layout_key='c23';
insert into public.apiary_hives(save_id,cell_id,installed_day)
select (select save_id from apiary_day_test_context),id,1 from public.garden_cells where save_id=(select save_id from apiary_day_test_context) and layout_key='c23';
select private.sync_garden_cells((select save_id from apiary_day_test_context),
  array[(select id from public.garden_cells where save_id=(select save_id from apiary_day_test_context) and layout_key='c23')]);
insert into public.apiary_colonies(save_id,hive_id,nosema_pressure,established_day)
select (select save_id from apiary_day_test_context),h.id,40,1 from public.apiary_hives h
join public.garden_cells c on c.id=h.cell_id and c.save_id=h.save_id where h.save_id=(select save_id from apiary_day_test_context) and c.layout_key='c23';
...
select ok((select private.hex_distance(target.col,target.row,source.col,source.row)>4
  ...
  where target.save_id=(select save_id from apiary_day_test_context) and source.save_id=target.save_id and target.layout_key='c2' and source.layout_key='c23'),
...
where ac.save_id=(select save_id from apiary_day_test_context) and ac.hive_id=h.id and h.save_id=ac.save_id and c.save_id=h.save_id and c.layout_key='c2';
update public.garden_plants set lifecycle='flowering',growth_progress=75,flowering_days_remaining=3
where save_id=(select save_id from apiary_day_test_context) and cell_id=(select id from public.garden_cells where save_id=(select save_id from apiary_day_test_context) and layout_key='c1');
...
select is((select (colony->>'floralHoney')::integer
  from jsonb_array_elements(private.resolve_garden_day((select save_id from apiary_day_test_context),1)->'colonies') colony
  where (colony->>'hiveId')::uuid=(select h.id from public.apiary_hives h join public.garden_cells c on c.id=h.cell_id and c.save_id=h.save_id where h.save_id=(select save_id from apiary_day_test_context) and c.layout_key='c2')),
...
select lives_ok($$ select public.advance_tavern_day(
  (select save_id from apiary_day_test_context),'16800000-0000-4000-8000-000000000010',0) $$,
  'permanent colony loss commits through the day boundary');
reset role;
select is((select count(*) from public.apiary_colonies ac join public.apiary_hives h on h.id=ac.hive_id and h.save_id=ac.save_id
  join public.garden_cells c on c.id=h.cell_id and c.save_id=h.save_id where ac.save_id=(select save_id from apiary_day_test_context) and c.layout_key='c2'),0::bigint,'lost colony is removed permanently');
select is((select count(*) from public.apiary_hives h join public.garden_cells c on c.id=h.cell_id and c.save_id=h.save_id
  where h.save_id=(select save_id from apiary_day_test_context) and c.layout_key='c2'),1::bigint,'colony loss leaves hive equipment available for replacement');
