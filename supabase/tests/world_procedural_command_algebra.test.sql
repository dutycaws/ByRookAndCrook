...
-- Capability sources, rather than the historical evolution pins, are the
-- authority used by procedural command admission.  The fixture creates one
-- deliberately unprivileged *source* in this rollback-only test transaction.
-- The production table still makes that source immutable after it is installed;
-- replica mode is used solely to construct the controlled fixture.
set local session_replication_role=replica;
update private.world_resident_procedural_capability_sources set capability_envelope=jsonb_build_object(
  'allowedWorldEffects','[]'::jsonb,
  'allowedActions','[]'::jsonb,
  'allowedApproaches','[]'::jsonb,
  'allowedTargetKinds','[]'::jsonb,
  'socialCapabilities','[]'::jsonb,'irreversibleEffects','[]'::jsonb
) where instance_id=(select unprivileged_resident_id from pg_temp.fixture);
set local session_replication_role=origin;
select ok((select capability_envelope->'allowedWorldEffects'='[]'::jsonb from private.world_resident_procedural_capability_sources where instance_id=(select unprivileged_resident_id from pg_temp.fixture)), 'fixture installs an immutable source with no world-effect authority');
select ok(not (private.world_procedural_resident_capability((select save_id from pg_temp.fixture),(select unprivileged_resident_id from pg_temp.fixture))->'allowedWorldEffects' ? 'create_entity'), 'resolved procedural capability has no entity authority before negative commands');
select throws_ok($$update private.world_resident_procedural_capability_sources set capability_envelope='{}'::jsonb where instance_id=(select unprivileged_resident_id from pg_temp.fixture)$$, '55000', null, 'the unprivileged procedural capability source remains immutable');
...
select ok(not exists(select 1 from private.world_settlement_jobs j join pg_temp.invalid i on i.j=j.id where j.input_snapshot->'capabilities'->(select unprivileged_resident_id::text from pg_temp.fixture)->'allowedWorldEffects' ? 'create_entity'), 'every negative lease freezes the resolved unprivileged capability before command admission');
