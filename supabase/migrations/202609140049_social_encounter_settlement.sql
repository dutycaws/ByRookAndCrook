...
begin
 ...
 if j.id is null or s.id is null or j.settlement_id<>new.settlement_id or j.job_kind<>'social_encounter' or j.subject_instance_id is not null or s.save_id<>new.save_id or s.day_number<>new.day_number or new.first_instance_id>=new.second_instance_id
  or not exists(select 1 from private.world_npc_instances i join private.world_resident_profiles p on p.instance_id=i.id join private.world_resident_package_pins pin on pin.instance_id=i.id and pin.save_id=i.save_id join private.npc_version_resident_packages package on package.id=pin.package_id and package.package_hash=pin.package_hash where i.id=new.first_instance_id and i.save_id=new.save_id and p.save_id=new.save_id and i.status='active')
  or not exists(select 1 from private.world_npc_instances i join private.world_resident_profiles p on p.instance_id=i.id join private.world_resident_package_pins pin on pin.instance_id=i.id and pin.save_id=i.save_id join private.npc_version_resident_packages package on package.id=pin.package_id and package.package_hash=pin.package_hash where i.id=new.second_instance_id and i.save_id=new.save_id and p.save_id=new.save_id and i.status='active') then raise exception using errcode='23514',message='Social encounter must match its ordered active packaged settlement pair'; end if;
 return new;
end $$;
...
create function private.world_social_encounter_resident(p_save_id uuid,p_instance_id uuid) returns jsonb language sql stable security definer set search_path='' as $$
 select jsonb_build_object('residentId',i.id,'npcId',p.npc_id,'profileRevision',p.profile_revision,'profile',p.current_profile,
  ...
  'capability',package.capability_envelope)
 from private.world_npc_instances i join private.world_resident_profiles p on p.instance_id=i.id and p.save_id=p_save_id join private.world_resident_package_pins pin on pin.instance_id=i.id and pin.save_id=p_save_id join private.npc_version_resident_packages package on package.id=pin.package_id and package.package_hash=pin.package_hash where i.id=p_instance_id and i.save_id=p_save_id
$$;
...
begin
 ...
 select a.id first_id,b.id second_id into pair from private.world_npc_instances a join private.world_npc_instances b on a.id<b.id join private.world_resident_profiles ap on ap.instance_id=a.id and ap.save_id=s.save_id join private.world_resident_profiles bp on bp.instance_id=b.id and bp.save_id=s.save_id join private.world_resident_package_pins ax on ax.instance_id=a.id and ax.save_id=s.save_id join private.world_resident_package_pins bx on bx.instance_id=b.id and bx.save_id=s.save_id join private.npc_version_resident_packages ax_package on ax_package.id=ax.package_id and ax_package.package_hash=ax.package_hash join private.npc_version_resident_packages bx_package on bx_package.id=bx.package_id and bx_package.package_hash=bx.package_hash where a.save_id=s.save_id and b.save_id=s.save_id and a.status='active' and b.status='active' and exists(select 1 from jsonb_array_elements(private.world_day_close_evidence(s.save_id,a.id,s.day_number))) order by a.id,b.id limit 1;
...
begin
 ...
 if not exists(select 1 from private.world_resident_profiles p where p.instance_id=e.first_instance_id and p.save_id=s.save_id and p.profile_revision=(e.context#>>'{participants,0,profileRevision}')::bigint and p.current_profile=e.context#>'{participants,0,profile}')
  ...
  or not exists(select 1 from private.world_resident_package_pins pin join private.npc_version_resident_packages package on package.id=pin.package_id and package.package_hash=pin.package_hash where pin.instance_id=e.first_instance_id and pin.save_id=s.save_id and package.capability_envelope=e.context#>'{participants,0,capability}')
  or not exists(select 1 from private.world_resident_package_pins pin join private.npc_version_resident_packages package on package.id=pin.package_id and package.package_hash=pin.package_hash where pin.instance_id=e.second_instance_id and pin.save_id=s.save_id and package.capability_envelope=e.context#>'{participants,1,capability}') then raise sqlstate 'PT409' using message='Frozen participants are stale'; end if;
