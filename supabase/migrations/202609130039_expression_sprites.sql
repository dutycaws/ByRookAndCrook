-- Expression sprites v2.  This is deliberately additive: v1 singular portraits
-- remain readable as Neutral, while new drafts and versions retain a complete,
-- immutable slot map.
begin;

alter table private.npc_drafts
  add column if not exists selected_portrait_slots jsonb not null default '{}'::jsonb;
alter table private.npc_versions
  add column if not exists portrait_slots jsonb not null default '{}'::jsonb;

alter table private.npc_portrait_candidates
  alter column job_id drop not null,
  add column if not exists expression_slot text not null default 'neutral',
  add column if not exists source_provenance text not null default 'ai_generated',
  add column if not exists neutral_anchor_hash text,
  add column if not exists neutral_anchor_asset_id uuid references private.npc_assets(id),
  add column if not exists upload_job_id uuid;
alter table private.npc_portrait_candidates drop constraint if exists npc_portrait_candidates_state_check;
alter table private.npc_portrait_candidates add constraint npc_portrait_candidates_state_check
  check (state in ('generating','ready','failed','stale','selected','superseded','discarded','quarantined','purged'));
alter table private.npc_portrait_candidates
  add constraint npc_portrait_candidate_expression_slot_check
    check (expression_slot in ('neutral','happy','sad','angry','engaged','leaving')),
  add constraint npc_portrait_candidate_source_check
    check (source_provenance in ('author_upload','ai_generated')),
  add constraint npc_portrait_candidate_anchor_check
    check ((expression_slot='neutral' and neutral_anchor_hash is null and neutral_anchor_asset_id is null) or (expression_slot<>'neutral' and neutral_anchor_hash ~ '^[0-9a-f]{64}$' and neutral_anchor_asset_id is not null));
create index if not exists npc_portrait_candidates_draft_slot on private.npc_portrait_candidates(draft_id,expression_slot,created_at desc);

-- Existing v1 selections are Neutral selections.  The map stores hashes and
-- provenance so selecting a later Neutral can invalidate optional expressions
-- without rewriting their candidate records.
update private.npc_drafts d
set selected_portrait_slots=jsonb_build_object('neutral',jsonb_build_object(
  'candidateId',c.id,'assetId',a.id,'assetHash',a.sha256,'source',c.source_provenance
))
from private.npc_portrait_candidates c join private.npc_assets a on a.id=c.asset_id
where d.selected_portrait_asset_id=c.asset_id and d.selected_portrait_slots='{}'::jsonb;
update private.npc_versions v
set portrait_slots=jsonb_build_object('neutral',jsonb_build_object(
  'assetId',a.id,'assetHash',a.sha256,'source',coalesce(c.source_provenance,'ai_generated'),'fallbackFrom',null
))
from private.npc_assets a left join private.npc_portrait_candidates c on c.asset_id=a.id
where v.selected_portrait_asset_id=a.id and v.portrait_slots='{}'::jsonb;

create table private.npc_portrait_generation_attempts (
  id uuid primary key default extensions.gen_random_uuid(),
  job_id uuid not null references private.npc_generation_jobs(id) on delete cascade,
  candidate_id uuid not null unique references private.npc_portrait_candidates(id) on delete cascade,
  ordinal integer not null check (ordinal between 1 and 4),
  status text not null default 'queued' check (status in ('queued','claimed','dispatched','ready','failed','ambiguous','cancelled')),
  infrastructure_attempts integer not null default 0 check (infrastructure_attempts between 0 and 3),
  lease_token uuid, lease_expires_at timestamptz, last_heartbeat_at timestamptz,
  provider_dispatched_at timestamptz, completed_at timestamptz, error_code text,
  result jsonb not null default '{}'::jsonb, created_at timestamptz not null default now(),
  unique(job_id,ordinal)
);
create index npc_portrait_generation_attempts_claim on private.npc_portrait_generation_attempts(status,lease_expires_at,created_at);
insert into private.npc_portrait_generation_attempts(job_id,candidate_id,ordinal,status,completed_at,error_code)
select c.job_id,c.id,c.ordinal,
  case when c.state in ('ready','selected','superseded') then 'ready' when c.state in ('failed','quarantined','purged') then 'failed' else 'queued' end,
  case when c.state in ('ready','selected','superseded','failed','quarantined','purged') then coalesce(c.completed_at,now()) else null end,
  c.failure_code
from private.npc_portrait_candidates c where c.job_id is not null
on conflict(job_id,ordinal) do nothing;

create or replace function private.npc_expression_slots_valid(p_slots jsonb) returns boolean
language sql immutable set search_path='' as $$
  select jsonb_typeof(p_slots)='object'
    and not exists(select 1 from jsonb_object_keys(p_slots) k where k not in ('neutral','happy','sad','angry','engaged','leaving'))
$$;
...
begin
  ...
  if tg_op='UPDATE' and old.state in ('submitted','published','retired','rejected')
    and (to_jsonb(new)-'state'-'published_at') is distinct from (to_jsonb(old)-'state'-'published_at')
    then raise exception 'NPC version content is immutable once submitted'; end if;
  return new;
end $$;

-- Visual-sheet changes invalidate all sprite selections.  The retained v1
-- function only knew selected_portrait_asset_id, so extend it before authors
-- can submit a stale slot map.
...
-- Neutral wrapper, preserving existing callers and completed jobs.
create or replace function public.npc_author_request_portrait(p_npc_id uuid,p_expected_revision bigint,p_controls jsonb,p_alternatives integer,p_slot text)
...
begin
  ...
  if p_slot<>'neutral' then
    anchor:=d.selected_portrait_slots#>>'{neutral,assetHash}'; anchor_asset:=(d.selected_portrait_slots#>>'{neutral,assetId}')::uuid;
    if anchor is null or anchor_asset is null then raise sqlstate 'PT422' using message='Select Neutral before requesting an expression'; end if;
  end if;
  base:=public.npc_author_request_portrait(p_npc_id,p_expected_revision,p_controls,p_alternatives);
  job:=(base->>'jobId')::uuid;
...
create or replace function public.npc_author_register_uploaded_portrait(p_npc_id uuid,p_expected_revision bigint,p_slot text,p_metadata jsonb)
returns jsonb language plpgsql security definer set search_path='' as $$
...
begin
  ...
  if p_metadata->>'runtimeStorageKey' is null or p_metadata->>'masterStorageKey' is null or p_metadata->>'runtimeSha256' !~ '^[0-9a-f]{64}$'
    or p_metadata->>'masterSha256' !~ '^[0-9a-f]{64}$' or p_metadata->>'originalSha256' !~ '^[0-9a-f]{64}$' or p_metadata->>'mimeType'<>'image/webp' or p_metadata->>'masterMimeType'<>'image/png'
    or p_metadata->>'originalWidth' !~ '^[0-9]+$' or p_metadata->>'originalHeight' !~ '^[0-9]+$' or p_metadata->>'originalByteSize' !~ '^[0-9]+$'
    or (p_metadata->>'originalWidth')::integer not between 256 and 4096 or (p_metadata->>'originalHeight')::integer not between 256 and 4096 or (p_metadata->>'originalByteSize')::integer not between 1 and 10485760
    or (p_metadata->>'width')::integer<>1024 or (p_metadata->>'height')::integer<>1536 or (p_metadata->>'byteSize')::integer not between 1 and 500000 then
    raise sqlstate 'PT422' using message='Upload derivative metadata is invalid'; end if;
  ...
  values(null,p_npc_id,d.id,d.owner_id,asset,1,'ready',vhash,d.revision,p_slot,'author_upload',anchor,case when p_slot='neutral' then null else (d.selected_portrait_slots#>>'{neutral,assetId}')::uuid end,now()) returning id into candidate;
...
create or replace function public.npc_author_discard_portrait_candidate(p_npc_id uuid,p_expected_revision bigint,p_candidate_id uuid)
returns jsonb language plpgsql security definer set search_path='' as $$
declare d private.npc_drafts; c private.npc_portrait_candidates; asset private.npc_assets;
begin
  d:=private.npc_assert_draft_owner(p_npc_id,p_expected_revision);
  select * into c from private.npc_portrait_candidates where id=p_candidate_id and draft_id=d.id and owner_id=auth.uid() for update;
  if not found or c.asset_id is null or c.state='selected' or d.selected_portrait_slots @> jsonb_build_object(c.expression_slot,jsonb_build_object('candidateId',c.id)) then raise sqlstate 'PT422' using message='Selected sprite cannot be discarded'; end if;
  select * into asset from private.npc_assets where id=c.asset_id for update;
  if exists(select 1 from private.npc_versions where portrait_slots @> jsonb_build_object(c.expression_slot,jsonb_build_object('assetId',asset.id))) then raise sqlstate 'PT422' using message='Published sprite cannot be discarded'; end if;
  update private.npc_portrait_candidates set state='discarded',completed_at=now() where id=c.id;
  update private.npc_assets set media_state='purged' where id=asset.id;
  perform private.npc_governance_log(p_npc_id,'portrait_candidate_discarded',asset.id,jsonb_build_object('candidateId',c.id,'slot',c.expression_slot));
  return jsonb_build_object('candidateId',c.id,'discarded',true,'revision',d.revision);
end $$;

create or replace function private.npc_finalize_portrait_generation_jobs() returns void
language plpgsql security definer set search_path='' as $$
begin
  update private.npc_generation_jobs j
  set status=case when exists(select 1 from private.npc_portrait_generation_attempts a where a.job_id=j.id and a.status='ready') then 'completed' else 'failed' end,
      completed_at=coalesce(j.completed_at,now()),
      result=jsonb_build_object('candidateCount',(select count(*) from private.npc_portrait_generation_attempts a where a.job_id=j.id and a.status='ready'),'failedCount',(select count(*) from private.npc_portrait_generation_attempts a where a.job_id=j.id and a.status in ('failed','ambiguous','cancelled')))
  where j.request->>'kind'='portrait' and j.status in ('queued','running')
    and exists(select 1 from private.npc_portrait_generation_attempts a where a.job_id=j.id)
    and not exists(select 1 from private.npc_portrait_generation_attempts a where a.job_id=j.id and a.status not in ('ready','failed','ambiguous','cancelled'));
end $$;

create or replace function public.npc_portrait_claim_generation_attempt() returns jsonb
language plpgsql security definer set search_path='' as $$
declare a private.npc_portrait_generation_attempts; token uuid:=extensions.gen_random_uuid();
begin
  perform private.npc_assert_service();
  -- An expired post-dispatch call is ambiguous. It is explicitly terminal so a
  -- worker restart never silently charges a provider twice. Expired preflight
  -- leases can retry, but only three times.
  update private.npc_portrait_generation_attempts set status='ambiguous',error_code='PROVIDER_DISPATCH_AMBIGUOUS',completed_at=now(),lease_expires_at=null
    where status='dispatched' and lease_expires_at<now();
  update private.npc_portrait_candidates c set state='failed',failure_code='PROVIDER_DISPATCH_AMBIGUOUS',completed_at=now()
    from private.npc_portrait_generation_attempts att where att.candidate_id=c.id and att.status='ambiguous' and c.state='generating';
  update private.npc_portrait_generation_attempts set status='failed',error_code='INFRASTRUCTURE_RETRY_EXHAUSTED',completed_at=now(),lease_expires_at=null
    where status='claimed' and lease_expires_at<now() and infrastructure_attempts>=3;
  update private.npc_portrait_candidates c set state='failed',failure_code='INFRASTRUCTURE_RETRY_EXHAUSTED',completed_at=now()
    from private.npc_portrait_generation_attempts att where att.candidate_id=c.id and att.status='failed' and att.error_code='INFRASTRUCTURE_RETRY_EXHAUSTED' and c.state='generating';
  perform private.npc_finalize_portrait_generation_jobs();
  select * into a from private.npc_portrait_generation_attempts
  where status='queued' or (status='claimed' and lease_expires_at<now() and infrastructure_attempts<3)
  order by created_at limit 1 for update skip locked;
  if not found then return null; end if;
  update private.npc_portrait_generation_attempts set status='claimed',lease_token=token,lease_expires_at=now()+interval '5 minutes',last_heartbeat_at=now(),infrastructure_attempts=infrastructure_attempts+1 where id=a.id;
  update private.npc_generation_jobs set status='running' where id=a.job_id and status='queued';
  return jsonb_build_object('attemptId',a.id,'leaseToken',token,'jobId',a.job_id,'candidateId',a.candidate_id,'ordinal',a.ordinal,
    'request',(
      select jsonb_build_object('npcId',j.npc_id,'sheet',d.sheet,'controls',j.request#>'{payload,controls}',
        'visualInputHash',j.request#>>'{payload,visualInputHash}','slot',j.request#>>'{payload,slot}',
        'neutralAnchorHash',j.request#>>'{payload,neutralAnchorHash}','neutralAnchorAssetId',j.request#>>'{payload,neutralAnchorAssetId}','styleVersion',j.request#>>'{payload,styleVersion}',
        'referenceSetVersion',j.request#>>'{payload,referenceSetVersion}')
      from private.npc_generation_jobs j join private.npc_drafts d on d.id=(j.request#>>'{draftId}')::uuid where j.id=a.job_id
    ),
    'neutralAnchorAsset',(
      select jsonb_build_object('assetId',a0.id,'runtimeStorageKey',a0.storage_key,'masterStorageKey',a0.generation->>'masterStorageKey','runtimeSha256',a0.sha256,'masterSha256',a0.generation->>'masterSha256')
      from private.npc_assets a0 join private.npc_generation_jobs j0 on j0.id=a.job_id
      where a0.id=(j0.request#>>'{payload,neutralAnchorAssetId}')::uuid
        and a0.sha256=j0.request#>>'{payload,neutralAnchorHash}' and a0.media_state not in ('quarantined','purged')
    ));
end $$;

create or replace function public.npc_portrait_mark_generation_dispatched(p_attempt_id uuid,p_lease_token uuid) returns void
language plpgsql security definer set search_path='' as $$
begin
  perform private.npc_assert_service();
  update private.npc_portrait_generation_attempts set status='dispatched',provider_dispatched_at=now(),lease_expires_at=now()+interval '5 minutes',last_heartbeat_at=now()
  where id=p_attempt_id and lease_token=p_lease_token and status='claimed' and lease_expires_at>=now();
  if not found then raise sqlstate 'PT409' using message='Generation lease is unavailable'; end if;
end $$;

create or replace function public.npc_portrait_heartbeat_generation_attempt(p_attempt_id uuid,p_lease_token uuid) returns void
language plpgsql security definer set search_path='' as $$
begin
  perform private.npc_assert_service();
  update private.npc_portrait_generation_attempts set lease_expires_at=now()+interval '5 minutes',last_heartbeat_at=now()
  where id=p_attempt_id and lease_token=p_lease_token and status in ('claimed','dispatched') and lease_expires_at>=now();
  if not found then raise sqlstate 'PT409' using message='Generation lease is unavailable'; end if;
end $$;
...
-- grants.  It exposes only the event-safe job and ordinal progress fields.
...
begin
  select * into j from private.npc_generation_jobs where id=p_job_id and request->>'kind'='portrait';
  if not found or j.requested_by<>auth.uid() or not private.npc_is_owner(j.npc_id) then raise sqlstate 'PT404'; end if;
  return jsonb_build_object('jobId',j.id,'npcId',j.npc_id,'slot',j.request#>>'{payload,slot}','status',j.status,'errorCode',j.error_code,
    'alternatives',(select coalesce(jsonb_agg(jsonb_build_object('ordinal',a.ordinal,'stage',case when a.status in ('claimed','dispatched') then 'generating' when a.status='ready' then 'completed' else a.status end,'status',a.status,'candidateId',a.candidate_id,'errorCode',a.error_code) order by a.ordinal),'[]'::jsonb) from private.npc_portrait_generation_attempts a where a.job_id=j.id));
end $$;

-- A focused initial-load contract avoids widening the historical workspace
-- payload while giving the Creator Studio one owner-scoped source of truth.
...
revoke all on function public.npc_author_request_portrait(uuid,bigint,jsonb,integer,text),public.npc_author_select_portrait(uuid,bigint,text,uuid,boolean),public.npc_author_register_uploaded_portrait(uuid,bigint,text,jsonb),public.npc_author_discard_portrait_candidate(uuid,bigint,uuid),public.npc_author_expression_sprite_workspace(uuid),public.npc_author_portrait_event_status(uuid),public.npc_portrait_claim_generation_attempt(),public.npc_portrait_mark_generation_dispatched(uuid,uuid),public.npc_portrait_heartbeat_generation_attempt(uuid,uuid),public.npc_portrait_complete_generation_attempt(uuid,uuid,jsonb,text) from public,anon,authenticated;
grant execute on function public.npc_author_request_portrait(uuid,bigint,jsonb,integer,text),public.npc_author_select_portrait(uuid,bigint,text,uuid,boolean),public.npc_author_discard_portrait_candidate(uuid,bigint,uuid),public.npc_author_expression_sprite_workspace(uuid),public.npc_author_portrait_event_status(uuid) to authenticated;
grant execute on function public.npc_author_register_uploaded_portrait(uuid,bigint,text,jsonb),public.npc_portrait_claim_generation_attempt(),public.npc_portrait_mark_generation_dispatched(uuid,uuid),public.npc_portrait_heartbeat_generation_attempt(uuid,uuid),public.npc_portrait_complete_generation_attempt(uuid,uuid,jsonb,text) to service_role;
