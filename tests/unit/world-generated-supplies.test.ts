...
const projection = () => ({ version:'generated-shop-v1', catalog:[{ entityId, itemKey:'road-provisions', name:'Road provisions', price:12, dailyStock:3, remainingStock:2 }], inventory:[{ entityId, itemKey:'road-provisions', name:'Road provisions', quantity:1 }] });

describe('generated supply projection', () => {
  it('accepts only the bounded player-facing catalog and inventory fields', () => {
    expect(parseGeneratedSupplies(projection())).toEqual(projection());
  });
  it('rejects private canonical mechanics and malformed finite values', () => {
    ...
    expect(() => parseGeneratedSupplies({ ...projection(), successorQuest:null })).toThrow('Invalid generated shop projection');
