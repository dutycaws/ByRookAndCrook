...
describe('world settlement worker', () => {
  ...
  it('discovers post-commit promoted NPCs after a fresh procedural commit without another provider call', async () => {
    ...
    const mock=client({world_settlement_commit_procedural_world:proceduralReceipt,world_materialize_procedural_npc_packages:{status:'completed',promotedCount:1}});
    expect(await runSettlementClaim(mock.api,proceduralClaim(),{provider,heartbeatMs:99_999,observability:(event)=>{events.push(event);}})).toEqual({status:'completed',kind:'procedural_world'});
    expect(provider.calls).toEqual(['procedural_world_proposer','procedural_world_critic']);
    expect(mock.calls.map((entry)=>entry.name)).toEqual(expect.arrayContaining(['world_settlement_commit_procedural_world','world_materialize_procedural_npc_packages']));
  ...
  it('retries the durable procedural receipt after an uncertain materialization response without provider work or fallback', async () => {
    ...
    const first=client({world_settlement_commit_procedural_world:proceduralReceipt,world_materialize_procedural_npc_packages:()=>({data:null,error:{message:'private promotion failure'}}),world_retry_procedural_npc_package_materialization:{status:'reused',promotedCount:0}});
    expect(await runSettlementClaim(first.api,proceduralClaim(),{provider:firstProvider,heartbeatMs:99_999,observability:(event)=>{failedEvents.push(event);}})).toEqual({status:'completed',kind:'procedural_world'});
    expect(first.calls.map((entry)=>entry.name)).toContain('world_retry_procedural_npc_package_materialization');
  ...
  it('reuses the durable command and package receipts after a worker replay without another provider call', async () => {
    ...
    const replayProvider=fixtureProvider({}); const replay=client({world_settlement_commit_procedural_world:(args:Record<string,unknown>)=>proceduralReceipt(args,true),world_materialize_procedural_npc_packages:{status:'reused',promotedCount:0}});
    ...
    expect(replay.calls.map((entry)=>entry.name)).toEqual(expect.arrayContaining(['world_settlement_commit_procedural_world','world_materialize_procedural_npc_packages']));
