    ...
    {#if journal.questArchive.items.length}
      <section class="guest-section" aria-label="Quest archive">
        ...
        {#each journal.questArchive.items as quest (quest.id)}
          <article class="npc-news">
            <p class="eyebrow">Days {quest.activationDay}–{quest.terminalDay} · {quest.origin === 'authored_milestone' ? 'Authored quest' : 'Successor quest'} · {quest.outcome}</p>
            <h3>{quest.title}</h3><p>{quest.objective}</p>
