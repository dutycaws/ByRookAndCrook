...
export function dialogueRuntime() {
  ...
  return {client:createClient<Database>(getSupabaseConfig().url,env.SUPABASE_SERVICE_ROLE_KEY,{auth:{persistSession:false,autoRefreshToken:false}}),
    provider:createProvider(env),options:{maxCalls:Number(env.NPC_MAX_CALLS)||8,rounds:Number(env.NPC_INVESTIGATION_ROUNDS)||2,deadlineMs:Number(env.NPC_DEADLINE_MS)||90000,observability:localAiObservabilitySink}};
}
