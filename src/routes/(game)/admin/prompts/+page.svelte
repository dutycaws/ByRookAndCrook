...
<main class="page-shell community-page prompt-registry-page">
  <PrivilegedWorkspaceNav capabilities={data.community.capabilities} current="prompts" />
  ...
  <section class="prompt-registry-workbench" aria-label="Prompt registry workspace">
    ...
    <section class="prompt-detail-panel workspace-panel" aria-labelledby="prompt-detail-heading">
      {#if activePrompt && activeRevision}
        ...
        <div class="prompt-view-controls" role="group" aria-label="Selected prompt view"><button class:secondary-action={activeView !== 'body'} type="button" aria-pressed={activeView === 'body'} onclick={() => activeView = 'body'}>Active body</button><button class:secondary-action={activeView !== 'edit'} type="button" aria-pressed={activeView === 'edit'} onclick={openEditor}>Create revision</button><button class:secondary-action={activeView !== 'history'} type="button" aria-pressed={activeView === 'history'} onclick={() => activeView = 'history'}>History & compare</button><button class:secondary-action={activeView !== 'workflow'} type="button" aria-pressed={activeView === 'workflow'} onclick={() => activeView = 'workflow'}>Workflow context</button></div>
        ...
        {:else if activeView === 'history'}
          <section class="prompt-history" aria-label="Prompt revision history">
            ...
            <div class="prompt-diff" aria-label="Unified prompt diff">{#each diffRows(activeBody, editorBody || activeBody) as row (row.line)}<div class:changed={row.changed} class="prompt-diff-row"><span class="prompt-diff-line" aria-label={`Line ${row.line}`}>{row.line}</span><del class="prompt-diff-before" aria-label="Before">{row.before || ' '}</del><ins class="prompt-diff-after" aria-label="After">{row.after || ' '}</ins></div>{/each}</div>
    ...
    <aside class="prompt-release-tray workspace-panel" aria-labelledby="release-tray-heading">
      ...
      <div class="prompt-staged-list prompt-staged-release-access">{#each stagedCandidates as candidate (candidate.key)}<article><div><strong>{candidate.key}</strong><small>Candidate r{candidate.revision}</small>{#if candidate.warningCodes.length}<em>Safety acknowledgement required</em>{/if}</div><button class="secondary-action" type="button" aria-label={`Remove ${candidate.key} from release tray`} onclick={() => { const copy = { ...staged }; delete copy[candidate.key]; staged = copy; }}>Remove</button></article>{:else}<p class="workspace-empty">No candidates are staged. Candidate revisions remain immutable even if removed from this tray.</p>{/each}</div>
      {#if stagedCandidates.length}
        <form method="POST" action="?/activate" use:enhance={activationEnhance()} class="prompt-activation-form prompt-staged-release-access">
